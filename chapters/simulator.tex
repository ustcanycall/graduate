...
\zadd{我们采用模拟的方法对新型加速器进行性能分析。传统的周期精确模拟器（cycle-accurate simulator, 简称Simulator-A）由于性能低，速度慢，无法满足我们实际的需求。因此，我们在周期精确模拟器的基础上进行性能优化，设计针对Cambricon-S的优化模拟器（optimized simulator, 简称Simulator-O），它能够在误差允许范围内快速进行性能模拟。
}
...
\section{本章小结}
\zadd{在本章，我们针对Cambricon-S设计了一个性能优化的模拟器，它基于事件触发的方式进行设计，能够在误差允许范围内快速对加速器进行模拟和性能评估。
}
